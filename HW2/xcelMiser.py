...
def run(strat):
    ...
    for index, room in enumerate(b.roomList):
        ...
        table.nextTable()
    initAvgStdDev = (b.getStdDevTemp() + b.getStdDevHumid()) / 2
    ...
    return [numExpand, numStored, attrList, initAvgStdDev]
...
def main():
    ...
    for i in range(100):
        ...
        # print("------------------------")
        details = run(strat)
        print("Time Complex: " + str(details[0]))
        print("Space Complex: " + str(details[1]))
        lsVisited.append(details[2][4])
        lsPower.append(details[2][5])
        lsTime.append(details[0])
        lsSpace.append(details[1])
        lsDev.append(details[3])
        print("\n")

    workbook = xlsxwriter.Workbook("toGraph1.xlsx")
    ...
    row = 1
    for p in lsPower:
        worksheet.write(row, 3, p)
        row += 1
    workbook.close()

    overall = [getListAverage(lsVisited),getListAverage(lsPower),getListAverage(lsTime),getListAverage(lsSpace)]
    # print("\n")
    # print("HeatMiser Details over 100 Simulations")
    print(tabulate([overall], headers=["Overall Average Rooms Visited", "Overall Average Power Used", "Overall Average Time Used", "Overall Average Space Used"], tablefmt='grid'))
    return
